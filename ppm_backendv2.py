...
LOAD IN THE DATA 
"""
# Change directory to data folder and read in the data
subdirectory_path = os.path.join(os.getcwd(), 'feature_importance')
os.makedirs(subdirectory_path, exist_ok=True)
df = pd.read_csv('CleanWageData.csv',  low_memory = False)
...
def walk_backward(early_year, late_year, years_extra, window, data, yearid, outcome, outcome_logged, excludevars, featureimportance, skills, regions, lgb_params, use_bootstrap, bootstrap_iterations, path2, filename2):
...
def run_pmtp(year_start, year_end, years_extra, window_size, path, filename, path2, filename2, year_id, outcome_var, outcome_logged, exclude_vars, feature_importance, split1, split2, lgb_params, use_bootstrap, bootstrap_iterations):
    os.chdir(path)
    ...
    if feature_importance == 1:
        subdirectory_path = os.path.join(path2, 'feature_importance')
        os.makedirs(subdirectory_path, exist_ok=True)
    results_1 = walk_backward(year_start, year_end, years_extra, window_size, df, year_id, outcome_var, outcome_logged, exclude_vars, feature_importance, split1, split2, lgb_params, use_bootstrap, bootstrap_iterations, path2, filename2)
    results_1.to_csv(filename2, index=False)
